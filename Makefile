...
init-vlc:
	...
	@cp ./twitch-extension.lua ~/.local/share/vlc/lua/playlist/

live-screenshot:
	@cvlc $(TWITCH) --rate=1 --video-filter=scene --vout=dummy --start-time=10 --stop-time=11 --scene-format=png --scene-ratio=24 --scene-prefix=snap --scene-path=`pwd`/imgs
